...
interface DailyInitial {
  name: string
  partnerName: string
  myMood: { emoji: string; text: string | null } | null
  partnerMood: { emoji: string; text: string | null; at: string } | null
  pulse: PulseData | null
  challenges: Challenge[]
  warmth: Array<{ id: string; text: string; fromName: string; fromId: string; createdAt: string }>
  rituals: Array<{ id: string; title: string; emoji: string; mine: boolean; partner: boolean }>
}
...
export default function DailyWidget({ initial }: { initial: DailyInitial }) {
  const router = useRouter()
  const [name, setName] = useState(initial.name)
  const [myMood, setMyMood] = useState<{ emoji: string; text: string | null } | null>(initial.myMood)
  const [partner, setPartner] = useState<{ name: string; mood: { emoji: string; text: string | null; at: string } | null } | null>(
    { name: initial.partnerName, mood: initial.partnerMood }
  )

  const [pulse, setPulse] = useState<PulseData | null>(initial.pulse)
  ...
  const [warmth, setWarmth] = useState<Array<{ id: string; text: string; fromName: string; fromId: string; createdAt: string }>>(initial.warmth)
  const [warmthText, setWarmthText] = useState('')

  const [rituals, setRituals] = useState<Array<{ id: string; title: string; emoji: string; mine: boolean; partner: boolean }>>(initial.rituals)
