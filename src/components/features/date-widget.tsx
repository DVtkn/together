...
interface DateInitial {
  me: { id: string; name: string | null } | null
  partnerName: string
  hasCouple: boolean
  invites: Invite[]
  memories: Array<{ id: string; venueName: string; date: string; photoUrl: string | null; note: string | null }>
}

export default function DateWidget({ initial }: { initial: DateInitial }) {
  const [loading, setLoading] = useState(false)
  const [me, setMe] = useState<{ id: string; name: string | null } | null>(initial.me)
  const [partnerName, setPartnerName] = useState(initial.partnerName)
  const [hasCouple, setHasCouple] = useState(initial.hasCouple)
  ...
  const [memories, setMemories] = useState<Array<{ id: string; venueName: string; date: string; photoUrl: string | null; note: string | null }>>(initial.memories)
