...
export default function CoupleWidget({ initial }: { initial: { status: Status | null; analytics: AnalyticsData | null } }) {
  const [s, setS] = useState<Status | null>(initial.status)
  const [an, setAn] = useState<AnalyticsData | null>(initial.analytics)
    ...
    <DashboardLayout>
      ...
      {partner && s.couple?.status === 'ACTIVE' && (
        <>
          ...
          <div className="k" id="synastry" style={{ scrollMarginTop: 80 }}>
            <span>Синастрия</span>
            <span className="badge-fun">🎲 развлекательное</span>
          </div>
      ...
      {/* ==== КОЛОДЫ БЛИЗОСТИ переехали в Психолога (/dashboard/ai) ==== */}
