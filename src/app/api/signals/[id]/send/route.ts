...
export async function POST(
  ...
  if (ctx.partner) {
    ...
    const replyHref = `/dashboard/ai?reply=${encodeURIComponent(signal.suggestedReply)}&signal=${encodeURIComponent(signal.emoji)}&meaning=${encodeURIComponent(signal.meaning)}`
